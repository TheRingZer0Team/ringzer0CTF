...
def ShowChallengesSolvedByUser():
    ...
    if UserID['success'] == 1:
        ...
        if len(List) != 0:
            ...
            print ('\n Total points for \'{}\' is {}. {} Challenge solved.\n'.format(Username, 0, 0))
    else:
        raise Exception ('\nOups! ID or username \'{}\' not found. Please retry.\n'.format(User))
    ...
    if(challenge['success'] == 1):
        ...
        for User in range(len(Users['writeup'])):
            if(int(Users['writeup'][User]['specialmention']) == 1):
                List.append([Users['writeup'][User]['receivedtime'], Users['writeup'][User]['user'],'Special mention'])
            else:
                List.append([Users['writeup'][User]['receivedtime'], Users['writeup'][User]['user'],''])
        List = (sorted(List))
        WriteUp = []
        for i in range(len(List)):
            WriteUp.append([i+1, List[i][0],  List[i][1],  List[i][2]])
